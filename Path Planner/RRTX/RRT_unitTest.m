...
for s = 1:length(scene_ids)
    ...
    for i = 1:num_iterations
        ...
        if goal_reached
            path_lengths(i) = rrt.getPathLength(); 
        else
            path_lengths(i) = NaN;
        end
    ...
    mysim.rrt(s).time = times;
    mysim.rrt(s).pathlength = path_lengths;
    mysim.rrt(s).avgtime = average_time;
    mysim.rrt(s).avgpathlength = average_path_length;
    mysim.rrt(s).stdtime = std_time;
    mysim.rrt(s).stdpathlength = std_path_length;
end

save('mysim_rrt.mat', 'mysim');